...
class HttpTestCase(testutils.TestCase):
    ...
    async def test_with_specific_appid(self) -> None:
        ...
        """
        # Arrange
        method = self._setup_apns_pushkin_snotif()
        ...
        resp = await self._request(
            self._make_dummy_notification([DEVICE_EXAMPLE_SPECIFIC])
        )
        ...
        assert {"rejected": []} == resp

    async def test_with_matching_appid(self) -> None:
        """
        ...
        # Act
        resp = await self._request(
            self._make_dummy_notification([DEVICE_EXAMPLE_MATCHING])
        )
    ...
    async def test_with_ambigious_appid(self) -> None:
        """
        ...
        # Act
        resp = await self._request(
            self._make_dummy_notification([DEVICE_EXAMPLE_AMBIGIOUS])
        )

        # Assert
        # must be rejected without calling the method
        assert 0 == method.call_count
        assert {"rejected": ["spqr"]} == resp
